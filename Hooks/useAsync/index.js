...
export const useAsync = (request, options = {
  ...
  const run =
    ...
    }, options.debounceInterval, { leading: true });
...
const FirstLoading = React.memo(({ children, count, size, loading }) => {
  if (count === 0) {
    return size === "small" ? (
...
export const AsyncLoading = React.memo(({ request, instance, children, options = { }, changeCallback, ...restProps }) => {
    ...
    <div {...restProps}>
      ...
      <FirstLoading size={size} {...instance.current}>
        <Spin spinning={loading}>
          {(isPreset(data) && (typeof children === "function" ? children(data, instance.current) : children)) ||
...
export const useAsyncLoading = (request, options) => {
  ...
  const loading = useMemo(() => ({children, ...restProps}) => <AsyncLoading request={request} instance={instance} options={options} {...restProps} children={children}/>, [changeFlag && changeFlag()])
